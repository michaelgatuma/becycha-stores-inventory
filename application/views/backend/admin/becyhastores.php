...
<!----CREATION FORM STARTS---->
                	...
                	<?php echo form_open(base_url() . 'admin/becyhastores/create' , array('class' => 'form-horizontal form-groups-bordered validate','target'=>'_top'));?>
			...
			<!----CREATION FORM ENDS-->
                    ...
                    <?php $counter = 1; $dormitories =  $this->db->get('becyhastores')->result_array();
                    foreach($dormitories as $key => $becyhastores):?>         
                        <tr>
                            <td><?php echo $counter++;?></td>
							<td><?php echo $becyhastores['name'];?></td>
							<td><?php echo $this->crud_model->get_type_name_by_id('hostel_category', $becyhastores['hostel_category_id']);?></td>
							<td><?php echo $this->crud_model->get_type_name_by_id('hostel_room', $becyhastores['hostel_room_id']);?></td>
							<td><?php echo $becyhastores['capacity'];?></td>
                            <td><?php echo $becyhastores['address'];?></td>
                            <td><?php echo $becyhastores['description'];?></td>
                            <td>
							
				    <a href="#" onclick="showAjaxModal('<?php echo base_url();?>modal/popup/edit_dormitory/<?php echo $becyhastores['dormitory_id'];?>');"><button type="button" class="btn btn-info btn-circle btn-xs"><i class="fa fa-pencil"></i></button></a>
					 <a href="#" onclick="confirm_modal('<?php echo base_url();?>admin/becyhastores/delete/<?php echo $becyhastores['dormitory_id'];?>');"><button type="button" class="btn btn-danger btn-circle btn-xs"><i class="fa fa-times"></i></button></a>
